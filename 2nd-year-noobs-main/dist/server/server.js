...
import { Repository, CollaborativeDocument, CollaborativeChat, ObjectStore, runHackathonDemo, computeSemanticDiff, detectStateConflicts, } from '../index.js';
...
function getOrCreateSession(branchName, peerId) {
...
app.get('/api/repo/state', (req, res) => {
...
app.post('/api/branch/checkout', (req, res) => {
...
app.post('/api/commit', (req, res) => {
    const { branch = repo.head.target, peerId = 'alice', message = 'feat: collaborative checkpoint', authorName = 'Alice', email = 'alice@example.com', } = req.body;
    try {
...
app.get('/api/commit/:id', (req, res) => {
    ...
    if (!commit) {
        ...
        if (match)
            commit = repo.getCommit(match.id);
    }
    if (!commit) {
        return res.status(404).json({ error: 'Commit not found' });
    }
    const tree = repo.getTree(commit.treeId);
    const entries = tree ? tree.listEntries().map((e) => {
...
function getCommitDocumentState(commitId) {
    ...
    const tree = repo.getTree(commit.treeId);
    const stateEntry = tree?.listEntries().find((e) => e.name === 'state.json');
    if (stateEntry) {
        ...
        if (blob) {
            try {
                ...
                if (typeof parsed.content === 'string') {
                    ...
                    text = parsed.content.map((c) => typeof c === 'string' ? c : c?.char || '').join('');
                }
                const metadata = {};
...
app.get('/api/branch/merge/preview', (req, res) => {
    const targetBranch = req.query.targetBranch || 'main';
...
app.post('/api/branch/merge', (req, res) => {
    const { targetBranch = 'main', sourceBranch, authorName = 'Automated Merge Engine', email = 'merge@gitcrdt.ai', message, } = req.body;
    if (!sourceBranch) {
        return res.status(400).json({ error: 'sourceBranch is required' });
    }
    try {
...
app.get('/api/session/state', (req, res) => {
    const branch = req.query.branch || (repo.head.type === 'branch' ? repo.head.target : 'main');
    const peerId = req.query.peerId || 'alice';
...
app.post('/api/session/edit', (req, res) => {
    const { branch = 'main', peerId = 'alice', type, text, position = 0, title, metadataKey, metadataVal, key, value, } = req.body;
...
// 13. Ingest Markdown
app.post('/api/ingest/markdown', upload.single('file'), async (req, res) => {
    try {
        let content = '';
...
app.post('/api/ingest/chat/chatgpt', upload.single('file'), async (req, res) => {
    try {
        let jsonStr = '';
        if (req.file) {
...
app.post('/api/ingest/chat/claude', upload.single('file'), async (req, res) => {
    try {
        let jsonStr = '';
        if (req.file) {
            jsonStr = req.file.buffer.toString('utf-8');
