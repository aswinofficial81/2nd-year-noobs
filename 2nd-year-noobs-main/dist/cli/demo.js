import { Repository, CollaborativeDocument, CollaborativeChat, ObjectStore, } from '../index.js';
 ...
 */
export async function runHackathonDemo(outputDir) {
    const steps = [];
    ...
    docBob.insertText(' Methodology: CRDT + Git version control.', docBob.getText().length);
    ...
    steps.push({
        ...
        details: `Messages: ${chatAlice.getMessages().length}, Reactions: ${JSON.stringify(chatAlice.getMessages()[0].reactions)}`,
    });
    // 5. Checkpoint v1.0 into Git Commit
    const commitV1 = repo.checkpointSession(sessionAlice, authorAlice, 'feat: release v1.0 whitepaper & chat');
    steps.push({
    ...
    const commitEval = repo.checkpointSession(sessionEval, authorBob, 'feat: add benchmark metrics');
    steps.push({
    ...
    const commitMain = repo.checkpointSession(sessionMain, authorAlice, 'feat: update publication status');
    steps.push({
    ...
    steps.push({
        ...
        details: `Merge Commit: ${mergeResult.commit?.id}, Parents: ${JSON.stringify(mergeResult.commit?.parentIds)}`,
