...
import { mergeSessions, threeWayStateMerge } from "../utils/mergeEngine.js";
import type {
  GitObject,
  HeadReference,
  AuthorSignature,
...
export class Repository {
  ...
  private _objects: Map<string, GitObject> = new Map();
  private _branches: Map<string, Branch> = new Map();
  private _head: HeadReference;
  private _sessions: Map<string, CRDTSession> = new Map();
   ...
   */
  public storeObject<T extends GitObject>(object: T): T {
   ...
   */
  public getObject(id: string): GitObject | undefined {
   ...
   */
  public getBlob(id: string): Blob | undefined {
   ...
   */
  public getTree(id: string): Tree | undefined {
    const obj = this.getObject(id);
   ...
   */
  public createBranch(name: string, startCommitId?: string): Branch {
   ...
   */
  public get head(): Readonly<HeadReference> {
    return { ...this._head };
  }
   ...
   */
  public get currentCommitId(): string | null {
   ...
   */
  public get currentCommit(): Commit | undefined {
  ...
  public checkout(
    target: string,
    options: { createBranch?: boolean; detach?: boolean } = {},
  ): void {
   ...
   */
  public createCommit(
    params: Omit<CommitParams, "parentIds"> & { parentIds?: string[] },
  ): Commit {
    const parentIds =
      params.parentIds ?? (this.currentCommitId ? [this.currentCommitId] : []);
  ...
  public createCRDTSession(
    ...
    if (!this._branches.has(branchName)) {
      throw new Error(
        `Cannot start CRDTSession on non-existent branch '${branchName}'`,
      );
    }
  ...
  public checkpointSession(
    ...
    if (registeredSession !== session) {
      throw new Error(
        `CRDTSession '${session.sessionId}' is not registered in this repository`,
      );
    }
    ...
    if (!branch) {
      throw new Error(
        `Cannot checkpoint session: branch '${session.branchName}' does not exist`,
      );
    }
    ...
    if (
      this._head.type !== "branch" ||
      this._head.target !== session.branchName
    ) {
      throw new Error(
        `Cannot checkpoint session: branch '${session.branchName}' is not checked out`,
      );
    }
   ...
   */
  public createSessionFromCommit(params: {
    commitId: string;
    peerId: string;
    branchName?: string;
    sessionId?: string;
   ...
   */
  public getCRDTSession(sessionId: string): CRDTSession | undefined {
   ...
   */
  public listCRDTSessions(): CRDTSession[] {
   ...
   */
  public closeCRDTSession(sessionId: string): boolean {
   ...
   */
  public toJSON(): RepositoryJSON {
    const blobs: ReturnType<Blob["toJSON"]>[] = [];
    const trees: ReturnType<Tree["toJSON"]>[] = [];
    const commits: ReturnType<Commit["toJSON"]>[] = [];

    for (const obj of this._objects.values()) {
      if (obj instanceof Blob) blobs.push(obj.toJSON());
      else if (obj instanceof Tree) trees.push(obj.toJSON());
      else if (obj instanceof Commit) commits.push(obj.toJSON());
    }
   ...
   */
  public static deserialize(serialized: string): Repository {
    const data = JSON.parse(serialized) as RepositoryJSON;
    return Repository.fromJSON(data);
  }
  public getCommitParents(commitId: string): Commit[] {
    ...
    return commit.parentIds
      ...
      .filter((parent): parent is Commit => parent !== undefined);
  }
  ...
  public isAncestor(ancestorId: string, commitId: string): boolean {
  ...
  public findCommonAncestor(
    ...
    let bestAncestor: Commit | undefined;
  ...
  public mergeBranches(params: {
    ...
    const mergedState = threeWayStateMerge(
      baseState,
      sessionOurs.exportState(),
      sessionTheirs.exportState(),
    );
   ...
   */
  public static fromJSON(json: RepositoryJSON): Repository {
